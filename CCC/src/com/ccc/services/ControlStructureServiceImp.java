...
public class ControlStructureServiceImp implements ControlStructureService {
	...
	public static void Calculation(CustomFile file) {
		...
		for (Line l : file.getLineSet()) {
			...
			// if contains any ITERATION in current statement assign weights and process
			// formula
			...
			// if current statement contains any CONDITONAL CS assign weights and process
			// formula
			...
			// if current statement includes any SWITCH, assign weights and calculate
			// complexity
			...
			if (line.contains("{") && line.contains("if") || line.contains("switch") || line.contains("for")
